...
public class Matchmaker
{
    ...
    public void Enqueue(ulong clientId, int difficultyLevel)
    {
        int existing = IndexOf(clientId);

        if (existing >= 0)
        {
            // Already waiting — just refresh the level rather than queueing them twice.
        ...
        queue.Add(new QueuedPlayer { clientId = clientId, difficultyLevel = difficultyLevel });
    }
    ...
    public List<List<ulong>> FormMatches(MatchMode mode)
    {
        ...
        List<QueuedPlayer> sorted = new List<QueuedPlayer>(queue);
    ...
    {
        int required = RequiredPlayersFor(mode);
        int remainder = queue.Count % required;
